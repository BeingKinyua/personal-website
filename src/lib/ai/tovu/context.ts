/**
 * Tovu Intelligence Layer — Context Assembly
...
export function assembleTovuPrompt(
  query: string,
  ...
  parts.push("=== TOVU SYSTEM CONTEXT ===");
  parts.push("Subject: Victor Kinyua (Software Architect & Full-Stack Systems Engineer)");
  parts.push("Platform: Tovu (Public Curated Web Intelligence & Architecture)");
  ...
  if (evidence.length > 0) {
    ...
    parts.push(
      "The following evidence records have been retrieved directly from Tovu's database and vector index. Base your answer strictly on these facts:"
    );
    parts.push("");
  ...
  if (history.length > 0) {
    ...
    for (const turn of recent) {
      parts.push(`${turn.role === "user" ? "USER" : "TOVU"}: ${turn.content}`);
    }
    parts.push("");
  }
  ...
  parts.push(
    "Respond with a single JSON object adhering strictly to the schema:\n" +
      JSON.stringify(
        {
          message: "Concise, precise, architectural response adhering to retrieved evidence.",
          references: [
            {
...
export const assembleDoomPrompt = assembleTovuPrompt;
