import { TovuRequest, TovuResponse, TovuAction, TovuReference } from "../types/tovu";
import { PROJECTS } from "../data/projects";
import { ARTICLES } from "../data/articles";
import { EXPERIMENTS } from "../data/experiments";

/**
 * Tovu — System Intelligence Layer
 *
 * Frontend service interface compliant with Supabase / RAG / real AI backend.
 * Provides structured responses, actions (navigate, open_project, open_article),
 * and reference entities.
 */
export async function askTovu(request: TovuRequest): Promise<TovuResponse> {
  const query = request.message.trim().toLowerCase();

  // Try real backend Tovu Intelligence RAG service
  try {
    const sessionId =
      sessionStorage.getItem("tovu_session_id") ||
      sessionStorage.getItem("victoros_session_id") ||
      `sess_${Date.now()}`;
    sessionStorage.setItem("tovu_session_id", sessionId);

    // Try canonical /api/v1/tovu, fallback to /api/tovu
    let res = await fetch("/api/v1/tovu", {
      method: "POST",
      headers: {
  ...
  // 1. AI Projects query
  if (
    query.includes("ai project") ||
    query.includes("machine learning") ||
    (query.includes("ai") && (query.includes("project") || query.includes("work")))
  ) {
    ...
    const references: TovuReference[] = aiProjects.map((p) => ({
      type: "project",
      id: p.id,
      title: p.title,
      slug: p.slug,
    ...
    return {
      message:
        `I located ${references.length} high-fidelity AI systems in the architecture records:\n\n` +
        aiProjects
          .map(
            (p) =>
              `• **${p.number} — ${p.title}** (${p.technologies.slice(0, 3).join(", ")})\n  ${p.subtitle}`
          )
          .join("\n\n") +
  ...
  if (
    ...
    return {
      message:
        `Victor is currently operating across three technical frontiers:\n\n` +
      ...
      references: [
        {
          ...
          title: "SSTables & Storage Engine Mechanics",
  ...
  if (
    ...
    return {
      message:
        `Navigating to the Journal module. Victor's writings center around:\n\n` +
  ...
  // 4. Who is Victor?
  if (
    query.includes("who is victor") ||
    query.includes("about victor") ||
    ...
    return {
      message:
        `Victor is a software engineer, AI builder, data thinker, and systems architect.\n\n` +
        `**Core Creed:**\n` +
  ...
  if (query.includes("nyayo")) {
    return {
      message:
        `**NYAYO** is Victor's flagship discipleship platform designed around knowing Christ and faithfully following Him.\n\n` +
      ...
      actions: [{ label: "Open NYAYO Full Case Study", type: "open_project", target: "nyayo" }],
      references: [
  ...
  if (
    ...
    query.includes("scout") ||
    query.includes("yolo") ||
    query.includes("kinematics")
  ) {
    return {
      message:
        `**Football Intelligence** is Victor's computer vision scouting pipeline:\n\n` +
      ...
      references: [
        {
          ...
          badge: "CV & Machine Learning",
        },
        {
          type: "experiment",
          id: "exp-07-football",
          title: "Experiment 07 - Pitch Reconstruction",
          slug: "exp-07-football",
          badge: "Lab Prototype",
        },
      ],
      timestamp: new Date().toLocaleTimeString(),
  ...
  return {
    message:
      `Tovu evaluated query: "${request.message}".\n\n` +
      `Tovu contains 5 completed engineering showcases, 5 active Lab experiments, 4 editorial essays, and 16 interconnected knowledge nodes. ` +
